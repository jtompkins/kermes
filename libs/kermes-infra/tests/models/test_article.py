import kermes_infra.models.article
import uuid
import datetime
import pytest
...
def mock_uuid(mocker):
    mocker.patch("kermes_infra.models.article.uuid4", return_value=uuid.UUID(EMPTY_UUID))
...
def mock_now(monkeypatch):
    ...
    monkeypatch.setattr(kermes_infra.models.article, "datetime", testdatetime)


def test_article_constructor_makes_well_formed_object(mock_uuid, mock_now):
    subject = kermes_infra.models.Article(EMPTY_UUID)
...
def test_article_to_dynamo_properly_handles_related_content(mock_uuid, mock_now):
    subject = kermes_infra.models.Article(EMPTY_UUID)
    subject.related_content.append(kermes_infra.models.article.RelatedContent("test_mime", "test_key"))
    dynamo_dict = subject.to_dynamo()
...
def test_article_from_dynamo_properly_converts_to_model(mock_uuid, mock_now):
    ...
    subject = kermes_infra.models.Article.from_dynamo(dynamo_dict)
